...
segments = journey_segments(visit_id, visits, unit_changes, units, services, patient_events, admission_chart)
admission_info = admission_chart[admission_chart["visit_id"] == visit_id]
if not admission_info.empty:
    admission_info = (
        admission_info.merge(
            units[["unit_id", "unit_name", "unit_service"]],
            left_on="admitted_unit_id",
            right_on="unit_id",
            how="left",
        )
        .merge(
            services[["service_id", "service_name"]],
            left_on="admitted_service_id",
            right_on="service_id",
            how="left",
        )
        .iloc[0]
...
details = {
    ...
    "Start Location": admission_info["unit_name"] if admission_info is not None else "Unavailable",
    "Start Service": admission_info["service_name"] if admission_info is not None else "Unavailable",
